...
- 爆款参考列表（可选，来自 2.1b：近一周垂类公众号文章，含 title/summary/datetime/source 账号名）
...
有爆款参考列表时，额外生成 **2-3 个爆款角度选题**。爆款和热点的本质区别：热点是
"正在发生的事"，爆款是"已被验证的内容需求"。爆款的正确用法是**拆解背后的逻辑，
然后模拟**——不是照抄某一篇，也不是为了不同而强行绕开。

搜狗结果不带阅读量，爆款信号从列表本身推断：
...
**每个爆款选题必须附一段「逻辑拆解」**——这是爆款池的核心产出，可迁移资产：
...
- **逻辑迁移**（把爆款的形式逻辑搬到自己垂类的另一个话题上）→ 不受同题密度约束，
  按新话题正常评分——这往往是爆款池最划算的用法

评分公式与热点选题一致（同题密度顶替热度项），标注"🔥 爆款"，与热点、常青混排。
...
- 热点选题 + 爆款角度选题 + 常青选题合计 10 个（无爆款参考数据时份额还给热点）
- 如果热点相关度 ≥ 5 的不足额定份额，用爆款角度或常青选题补足
